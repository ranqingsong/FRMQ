...
est_rec_prob = function(month_output_list, channel){
  ...
  pred_k1 = sapply(1:length(month_output_list), 
                   function(num_reg) 
                     assign(paste0("pred_", names(month_output_list)[num_reg]), month_output_list[[num_reg]]$fitted.values))
...
create_plot_two = function(month_output_stepwise, channel,title){
  ...
  plot(x=pred_1$ym, y=pred_1$rec, type="n", xaxt = "none",ylab = "Implied recession probability", xlab="", cex.lab=1.7, cex.axis=2)
...
#FRM@America k=1, probit regression
