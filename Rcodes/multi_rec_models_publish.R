...
rec_model_stepwise_all = function(channel){
  ...
  pm_full_iqr = glm(rec ~ lag1_FRM_iqr+lag2_FRM_iqr+lag3_FRM_iqr+ lag4_FRM_iqr +lag5_FRM_iqr+lag6_FRM_iqr, family = binomial(link = "probit"), data = data_m)
...
est_rec_prob = function(month_output_stepwise_all, channel){
  ...
  pred_k1 = sapply(1:length(month_output_stepwise_all), function(num_reg) assign(paste0("pred_", names(month_output_stepwise_all)[num_reg]), month_output_stepwise_all[[num_reg]]$fitted.values))
