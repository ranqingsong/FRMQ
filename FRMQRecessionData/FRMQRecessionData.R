...
create_lag_table <- function(date_start1, date_end1, channel) {
    data <- read.csv(paste0("FRM_index_all", date_start1, "-", date_end1, channel, ".csv"), colClasses = c("Date", "numeric", "numeric", 
    ...
    data_m = aggregate(cbind(FRM_index, FRM_q50, FRM_q60, FRM_q70, FRM_q80, FRM_q90, FRM_iqr) ~ ym, data = data, mean)
    ...
    write.csv(data_m, paste0("lagged_daily_FRM_index_all", date_start1, "-", date_end1, channel, ".csv"), row.names = FALSE, quote = FALSE)
    write.csv(data_m, paste0("lagged_FRM_index_all", date_start1, "-", date_end1, channel, ".csv"), row.names = FALSE, quote = FALSE)
}
