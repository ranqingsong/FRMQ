...
# setwd("")
setwd("~/studium/Masterarbeit_local/novel-FRM/FRMQcorrelation_AM")
...
#plot FRM@Americas, VIX, GT, FT
cols = brewer.pal(9, "Set1")
datecut = which(scaled_data$date==as.Date("2009-12-31","%Y-%m-%d"))
png(paste0("systemic_risk_ indicators_",channel,".png"), width = 1500, height = 1000, bg = "transparent")
par(mfrow =c(1,2))
...
png(paste0("systemic_risk_indicators2_",channel,".png"), width = 1500, height = 1000, bg = "transparent")
par(mfrow =c(1,2))

{plot(scaled_data$date[(datecut+1):length(scaled_data$date)],scaled_data$GT[(datecut+1):length(scaled_data$date)], lwd =3, type = "l",
      ylim = c(0,1), xlab = "", ylab = "nomalized systemic risk indictors", cex.axis=2, cex.lab=2, col="darkblue", xaxt="none")
