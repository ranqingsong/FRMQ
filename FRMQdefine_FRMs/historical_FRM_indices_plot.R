...
#import rds data
#calculate mean, quantiles(q50,q60,q70,q80,q90), and IQR of lambda 
...
create_boxplot <- function(date_start, date_end, channel){
  ...
  plot(x=FRM_index_all$ticker, y = FRM_index_all$FRM_iqr, type = "l",col = cols[7], lwd = 2, ylim = c(0,max(FRM_index_all$FRM_q90)),
       xlim = c(min(FRM_index_all$ticker), max(FRM_index_all$ticker)), xaxt="none", xlab = "", ylab = "FRM index", cex.lab=1.5, cex.axis=1.5)
