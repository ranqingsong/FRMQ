...
create_lag_table <- function(date_start1, date_end1, channel){
  data <- read.csv(paste0("01_csvData/FRM_index_all",date_start1,"-",date_end1, channel,".csv"), 
  ...
  if (channel== "Americas") {
    ...
    recdata<- read.csv("01_csvData/EUROREC.csv")
    recdata$date <- as.Date(recdata$DATE, format = "%Y-%m-%d")
    recdata$ym <- paste0(year(recdata$date), "-",format(recdata$date, "%m"))
    recdata <- rename(recdata, rec=EUROREC)
    recdata <- select(recdata, c("ym", "rec"))
  }
  #lag daily FRM candidates at 1 to 6 months(21 days per month) 
  data_d = data
  for (lag_num in 1:6) {
    lags_daily = sapply(2:8, function(i) dplyr::lag(data[,i],21*lag_num))
    collist = c("FRM_index","FRM_q50","FRM_q60","FRM_q70","FRM_q80","FRM_q90","FRM_iqr")
    colnames(lags_daily) = paste0("lag", lag_num,"_",collist)
    data_d = cbind(data_d, lags_daily)
  }
  ...
  write.csv(data_m, paste0("01_csvData/lagged_daily_FRM_index_all",date_start1,"-",date_end1, channel,".csv"),
            row.names = FALSE, quote = FALSE)
  write.csv(data_m, paste0("01_csvData/lagged_FRM_index_all",date_start1,"-",date_end1, channel,".csv"),
            row.names = FALSE, quote = FALSE)
  #return(data_m)
}
