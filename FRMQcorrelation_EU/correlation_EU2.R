...
## import FRM datasets
frm.data <- read.csv("FRM_all_EU.csv", colClasses = c("Date", "numeric", "numeric", "numeric", "numeric", "numeric", 
    "numeric", "numeric"))
...
daily.ciss <- seq(CISS$Date[1], tail(CISS$Date, 1), by = "day")
ciss.daily <- data.frame(date = daily_ciss, CISS_d = spline(CISS, method = "fmm", xout = daily_ciss)$y)
...
write.csv(scaled_data, file = "systemic_risk_comparison_EU.csv", row.names = FALSE, quote = FALSE)
...
par(mfrow = c(1, 2))
{
    plot(scaled_data$date[1:datecut], scaled_data$FRM_index[1:datecut], type = "l", xlab = "", ylab = "nomalized systemic risk indictors", 
        ylim = c(0, 1), lwd = 3, xaxt = "none", cex.axis = 2, cex.lab = 2)
...
{
    plot(scaled_data$date[1:datecut], scaled_data$FRM_q50[1:datecut], type = "l", lty = 1, xlab = "", ylab = "", ylim = c(0, 
        1), cex.axis = 2, cex.lab = 2, xaxt = "none", col = "darkred", lwd = 2)
...
par(mfrow = c(1, 2))
{
    plot(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_index[(datecut + 1):length(scaled_data$date)], 
        type = "l", ylim = c(0, 1), xlab = "", ylab = "nomalized systemic risk indictors", lwd = 3, cex.axis = 2, xaxt = "none", 
        cex.lab = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FT[(datecut + 1):length(scaled_data$date)], 
        type = "l", col = "violet", lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$VSTOXX[(datecut + 1):length(scaled_data$date)], 
        type = "l", col = "red", lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$CISS[(datecut + 1):length(scaled_data$date)], 
...
{
    plot(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_q50[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, xlab = "", ylab = "", ylim = c(0, 1), xaxt = "none", cex.axis = 2, cex.lab = 2, col = "darkred", 
        lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_iqr[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, col = cols[7], lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_q80[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, col = cols[4], lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_q90[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, col = cols[5], lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_q60[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, col = cols[2], lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_q70[(datecut + 1):length(scaled_data$date)], 
        type = "l", lty = 1, col = cols[3], lwd = 2)
    lines(scaled_data$date[(datecut + 1):length(scaled_data$date)], scaled_data$FRM_index[(datecut + 1):length(scaled_data$date)], 
